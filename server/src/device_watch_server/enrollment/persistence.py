...
def insert_device(connection: Connection, device: DeviceIdentity) -> None:
    ...
    connection.execute(devices_table.insert().values(
        device_id=str(device.device_id), display_name=device.display_name,
        created_at=_mysql_time(device.created_at), lifecycle=device.lifecycle.value,
    ))


def insert_credential(
    connection: Connection, device_id: UUID, credential: CredentialRecord,
) -> None:
    """Persist only the hash representation, never the issued secret carrier."""
    connection.execute(credentials_table.insert().values(
        key_id=credential.key_id, device_id=str(device_id),
        secret_hash=credential.secret_hash, created_at=_mysql_time(credential.created_at),
        last_used_at=None,
        revoked_at=None if credential.revoked_at is None else _mysql_time(credential.revoked_at),
        replaced_at=None if credential.replaced_at is None else _mysql_time(credential.replaced_at),
    ))
