...
## Current acceptance remap - 2026-09-12

Scope: Stage 1 acceptance remap only, using the latest V01-V06 evidence.

Revision reviewed: `f5ef9e3c7564049baa3852fedba621ace87db58b`, plus the
concurrent user changes to `.gitignore` and `deploy/version.env` described below.
Only this V07 evidence file was edited for the remap.

Overall result: **FAIL**.

Acceptance totals: **9 PASS, 1 FAIL, 6 BLOCKED BY ENVIRONMENT**.

- **PASS:** criteria **1, 4, 5, 6, 10, 11, 12, 14, 16**.
- **FAIL:** criterion **9**, due to the current Stage 1 repository-safeguard
  regression demonstrated below.
- **BLOCKED BY ENVIRONMENT:** criteria **2, 3, 7, 8, 13, 15**.
- **Deterministic Stage 1 implementation failure remains: YES.** Required
  local-database and operator-environment ignore protections are missing.

Criteria **10 and 11 are both PASS**, incorporating the latest targeted V06
re-verification. The safeguard failure is a separate current-file regression;
it does not reverse those repaired database-content and historical-scope results.
...
- [V01](01-repository-server-foundation.md): the R1 re-verification describes
  ...
  gap. Current manifest/lock inspection confirms `httpx` is test-only. The
  ignore-rule evidence required the lightweight checks below because that
  verified configuration changed again.
...
- [V05](05-deployment-topology.md): R4A/R4C deterministic repairs pass; the
  deployment suite records 12 passed and one Docker skip. Current `.gitignore`
  no longer supports the separately recorded operator-file ignore assertion.
...
Git path-history comparison with the previous V07 checkpoint `cd40866` showed
...
| 9 | Automated tests pass independently. | **FAIL** | The prescribed V01 repository-safeguard checks now fail deterministically: repository-only `git check-ignore` exits 1 for both local database examples and for `deploy/.env.prod`. Changed verified configuration invalidates the prior safeguard evidence. Existing Stage 1 component test passes are retained; their Docker/MySQL/supported-runtime blocks also remain. |
...
### Lightweight gap checks and remaining failure

V01's stale report required resolving the R1 repair history. `b6cda56` added
the required safeguards, but `.gitignore` subsequently lost local-database,
environment, key, virtualenv, cache, dependency, build, and coverage patterns.
The approved design explicitly requires these categories at line 368. Fresh
checks confirmed the following without creating sample files:

| Check | Result |
| --- | --- |
| `git -c core.excludesFile=NUL check-ignore --no-index -v -- device-watch.sqlite local.db` | **FAIL** — exit 1, no matches for either required local-database example. |
| `git -c core.excludesFile=NUL check-ignore --no-index -v -- deploy/.env.prod` | **FAIL** — exit 1, no match for the operator-managed environment file previously verified by V05/R4C. |
| Editor examples in the initial four-path check | **PASS** — `.vscode/settings.json` and `.idea/workspace.xml` still match repository rules. |
| Current `server/pyproject.toml` and server package entry in `server/uv.lock` | **PASS** — `httpx` is in the test group, absent from the server's direct runtime dependencies. No dependency installation or tree regeneration was performed. |
| Existing `_has_unsupported_database_url` rule applied only to changed `deploy/version.env` | **PASS** — exit 0; no unsupported database scheme. Used the existing server Python environment with host access and `-B`; no values were printed. |

Criterion 9 includes the prescribed automated repository checks in the
V01-V06 verification scope. Its FAIL here comes from a mandatory safeguard
assertion on changed Stage 1 configuration, not a rerun of the component test
suites. The failure is deterministic and must not be classified as an
environment block. V01 safeguard evidence and V05's operator-ignore assertion
need targeted repair/re-verification before a clean Stage 1 handoff.

Concurrent user changes added tool-related ignores and modified
`deploy/version.env`; the repeated safeguard checks above still failed against
that working state. Both files were preserved without editing. This remap does
not audit the operator values or deploy them.
...
the established source-focused type-check gate. No code/test repair was made.

### Retained environment blocks and V08 handoff

The exact criteria classified **BLOCKED BY ENVIRONMENT** are **2, 3, 7, 8,
13, and 15**. Criterion 9 additionally retains the unexecuted real-MySQL,
Docker-dependent test, and supported-runtime paths, although its proved
safeguard failure controls its overall FAIL classification. No Docker/MySQL/
Caddy/runtime check was upgraded to PASS based on static evidence.
...
**V07 is ready to feed a separately authorized V08 baseline regeneration, but
only a baseline that explicitly retains the current FAIL. Stage 1 is not ready
to be declared an accepted or safe foundation.** A final baseline with only
environment blocks requires the safeguard failure to be repaired and
re-verified first. `stage-01-baseline.md` was not regenerated in this session.
